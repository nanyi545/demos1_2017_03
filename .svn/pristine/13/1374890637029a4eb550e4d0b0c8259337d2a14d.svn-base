...
/**
 * 自定义按钮
 * <p>由进度条View和动画、按钮变形动画->组成全部的效果</p>
 * <p>说明：该组件导致不断分配内存，即使在程序退出的情况下，故已废弃</p>
 */
public class CircularProgressButton extends Button {
    ...
    private boolean mMorphingInProgress;	//进度条模式
    ...
    private void drawIndeterminateProgress(Canvas canvas) {
        if (mAnimatedDrawable == null) {//为空
            ...
            mAnimatedDrawable.start();				//启动
        } else {
            mAnimatedDrawable.draw(canvas);			//绘制
        }
    }
    ...
    private void drawProgress(Canvas canvas) {
        ...
        mProgressDrawable.setSweepAngle(sweepAngle);//设置弧长
        mProgressDrawable.draw(canvas);				//绘制
    }
    ...
    @Override
    protected boolean verifyDrawable(Drawable who) {//此操作允许进行绘制时有动画效果
    ...
    protected void removeIcon() {
        setCompoundDrawablesWithIntrinsicBounds(0, 0, 0, 0);//手动设置文本与图片相对位置时
        setPadding(0, 0, 0, 0);
    }
    ...
    public void setProgress(int progress) {
        ...
        mStateManager.saveProgress(this);//保存当前进度
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (changed) {
//        	Log.w("CircularProgressButton", "changed true");
            setProgress(mProgress);
        }else{
//        	Log.w("CircularProgressButton", "changed false");
        }
    }
