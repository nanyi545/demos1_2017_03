...
public class PublicMethodUtil {
	...
	private static final String DETAILED_ACTIVITY_NAME = "AlarmDetailsActivityCompat";
	...
	public AlertDialog.Builder makeAlertDialogBuilder(Context context, int titleResId, 
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(new ContextThemeWrapper(context, R.style.DialogThemeMaterial));
	 ...
	 */
	public long dateChangeNotificationId(String date, String time){
		String idStr = date.replace("-", "") + time.replace(":", "");
		//除以类型确保唯一性
		return Long.parseLong(idStr);
	}
	
	public long dateChangeNotificationId(String dateTime){
		String date = dateTime.substring(0, dateTime.indexOf(" "));
		String time = dateTime.substring(dateTime.indexOf(" ") + 1);
		return dateChangeNotificationId(date, time);
	}
}
