...
import com.webcon.sus.utils.OkHttpClientManager;
...
//						EncrypAES.getInstance().encrypt("")));
        ip = context.getResources().getString(R.string.str_update_default_ip);
        //--↑--test---↑----
...
//			Log.i("BBB", "version: " + curVersionName+"	Package name"+pInfo.packageName);
            if(WPApplication.DEBUG){
                Log.i("update>>", "self version: " + curVersionName);
            }
	...
	public void checkUpdate() {
		...
		new Thread() {
			@Override
			public void run() {
				Log.d("Thread_check", "checkUpdate----" + Thread.currentThread().getName());
				try {
					String url = "http://" + ip + WPConstant.VERSION_DOWNLOAD_PATH + "?type=phone";
					newVersionName = OkHttpClientManager.getInstance().getAsString(url).trim();
                    if(WPApplication.DEBUG){
                        Log.i("update>>", "latest version: " + newVersionName);
                    }
	...
	Handler updateHandler = new Handler() {
		...
		public void handleMessage(Message msg) {
			Log.d("Thread_check", "upDateHander----" + Thread.currentThread().getName());
			switch (msg.what) {
