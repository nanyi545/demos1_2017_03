package com.webcon.sus.handler;
...
//                builder.setMessage(ex.getMessage());
                builder.setMessage(R.string.dialog_logoutexit_message_crash);

                Log.e("unCaught", Log.getStackTraceString(ex));


                builder.setNegativeButton(
                        R.string.dialog_logoutexitreceiver_button,
    ...
    private String saveCatchInfo2File(Throwable ex) {
        ...
        try {
            ...
            if (Environment.getExternalStorageState().equals(
                ...
                File dir = new File(WPConstant.TEMP_LOG_PATH);
                if (!dir.exists()) {
                    dir.mkdirs();
                }
                File saveFile = new File(WPConstant.TEMP_LOG_PATH, fileName);
    ...
    private void sendCrashLog2PM(String fileName) {
        if (!new File(WPConstant.TEMP_LOG_PATH + fileName).exists()) {
    ...
    private MimeBodyPart createContentPart(String bodyStr, String filename)
        ...
        FileDataSource fds = new FileDataSource(WPConstant.TEMP_LOG_PATH
                + filename); // 打开要发送的文件
