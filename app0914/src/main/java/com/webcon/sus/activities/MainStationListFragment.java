...
public class MainStationListFragment extends Fragment
    ...
    //------delay the jump to show the ripple ----------
    Handler fragJumpHandler = new Handler() {
        @Override
        public void handleMessage(Message msg) {
            int position = msg.what - 987;
//            Log.i("fraghandler",""+position);
            Intent intent = new Intent(mContext, StationDetailsActivity.class);
            Bundle bundle = new Bundle();
            bundle.putInt(KEY_ID, getStationList().get(position).getId());
            Log.i("dataTest", "MainList--From--station id" + getStationList().get(position).getId());
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        Log.i("AAA", "new alarm numbers" + WPApplication.getInstance().getNewAlarms());

        // test Button
//        testB=(Button)layout.findViewById(R.id.testB);
//        testB.setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View v) {
//                CommunicationUtils.getInstance().refreshAllStationInformation(null);
//            }
//        });
//        Log.i("BBB",""+testB.getText());
...
//                        Log.i("MainStationListFragment","clicked--"+position);

                        // delay the activity jump to reveal the ripple ---
                        final int position_t = position + 987;
                        new Thread(new Runnable() {
                            ...
                            public void run() {
                                ...
                                fragJumpHandler.sendEmptyMessage(position_t);
                            }
                        }).start();
...
//                        getActivity().overridePendingTransition(R.anim.page_right_in, android.R.anim.fade_out);
                        ...
                        Snackbar.make(mSwipeLayout, "站场服务器当前未上线！", Snackbar.LENGTH_SHORT)
    ...
    private void stopWaiting() {
        if (WPApplication.DEBUG) {
            Log.w(TAG, "stop swipelayout refreshing!!");
        }
...
//        Log.i("StationEventHandle","StationEvent:type---"+event.getType());
    ...
    public void onEventMainThread(MessageEvent event) {
        switch (event.getType()) {
            case MessageEvent.ALARM_FLAG_REFRESH:
                ...
                refreshAdapter();
                break;
            case MessageEvent.ALARM_SWIPE_DELETED:
                //刷新列表
                refreshAdapter();
                break;
            default:
...
//        mSwipeLayout.setRefreshing(true);  // should not call this when refresh is triggered by gesture
        refreshExecutor();
        CommunicationUtils.getInstance().startInit(initExecutor);
