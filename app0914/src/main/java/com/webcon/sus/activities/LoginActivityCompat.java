...
//        ApplicationManager.getInstance().addActivity(this);
    ...
    private void initWelcomeView() {
        ...
        Log.d("Thread_check", "initWelcomeView----" + Thread.currentThread().getName());
...
//                WindowManager.LayoutParams.FLAG_FULLSCREEN));
        ...
        // 全局变量类实例化---> onCreate 已经执行过 WPApplication.getInstance()了，下面这句有必要么？
//        WPApplication.getInstance();
    ...
    private Runnable autoRun = new Runnable() {
        @Override
        public void run() {
            Log.d("Thread_check", "autoRun----" + Thread.currentThread().getName());
//            autoRun通过postDelayed启动，还是在main线程中，直接调用run方法
            isStartView = false;
    ...
    private void initLoginView() {
        ...
        //初始化组件
        linear_setting = (LinearLayout) findViewById(R.id.login_rl_setting);   //设置ip端口界面
    ...
    private Handler loginMessageHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            Log.i("AAA", "handle message:" + msg.what + "--true or false---" + ((msg.what == -4004) || (msg.what == -4010)));

            // 不需要重新init
            if (msg.what == -4004||msg.what == -4010 || msg.what == -4005 || msg.what == 9|| msg.what == -7) {
                if (loginLoading != null && loginLoading.isShowing()) {
                    loginLoading.dismiss();
                }
                // 用户名不存在
                if (msg.what == -4004||msg.what == -4010) {
                ...
                // 登陆超时
                    ...
                    showToast(mContext, R.string.str_login_toast_login_f7, 1000);
                }
                // 服务器IP填错
                else if (msg.what == 9) {
            ...
            // 服务器出错 无法自动登录
                ...
                // 可以自动登录的
                if (msg.what == 1 && !isAutoLogin && autoLoginLimited-- > 0) {
                    ...
                    if (loginLoading != null && loginLoading.isShowing()) {
                        Toast.makeText(mContext, "未知错误:" + msg.what, Toast.LENGTH_SHORT).show();
                        loginLoading.dismiss();
                    }
                }
            }
    ...
    private View.OnClickListener mClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            //默认网络参数按钮
                ...
                edit_ip.setText(getString(R.string.str_login_default_ip));  //默认ip地址
    ...
    private void setValueInUi() {
        ...
        String ip = getDecrypt(wpPreferences.getString(
                ...
                getEncrypt(NIL_STR)));
        if (!ip.equals("")) {  //读取sharedPreference
    ...
    private boolean loginVerify(String name, String pwd, String ip, String port) {
        if (name.length() == 0 || pwd.length() == 0) {
            ...
            if (ip.length() == 0 || port.length() == 0) {
               ...
               //  Toast.makeText(this,""+checkIp(ip)+"---ip--"+ip,Toast.LENGTH_LONG).show();
                if (checkIp(ip)) {
                    if (checkPort(port)) {
                        return true;
                    } else {
    ...
    private class LoginRunnable implements Runnable {
        ...
        private LoginRunnable(byte[] name, String pwd, String ip, int port) {
            ...
            this.strIp = ip;
            this.port = port;
            Log.i("login","ip:"+this.strIp+"  port"+this.port);
        }

        @Override
        public void run() {
            Message msg = loginMessageHandler.obtainMessage();  // ？？？干啥用的
            int initResult = 0;
...
//            Log.i("native","============== init > nret " + nret);
    ...
    @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {  //有用么？？？
