...
//			topView.setOnTouchListener(topViewTouchListener);
		...
		measureCount++;
		Log.i("ccc","measureCount:"+measureCount+"  getMeasuredHeight:"+getMeasuredHeight()+"   getMeasuredWidth:"+getMeasuredWidth());
	}
	...
	private void updatePullState(MotionEvent ev){
		topView.getHitRect(topRect);
		bottomView.getHitRect(bottomRect);

		if (topRect.contains((int)ev.getX(),(int)ev.getY())){
			ScrollView sv = (ScrollView) topView;
			boolean svContentSmallerThanSvHeight=sv.getChildAt(0).getMeasuredHeight() - sv.getMeasuredHeight() <0;
			boolean svContentLargerThanSvHeight_hit_bottom=sv.getChildAt(0).getMeasuredHeight() - sv.getMeasuredHeight()==sv.getScrollY();
			if ((  svContentSmallerThanSvHeight||svContentLargerThanSvHeight_hit_bottom )&& mCurrentViewIndex == 0)
				canPullUp = true;
			else
				canPullUp = false;
		}
		if (bottomRect.contains((int)ev.getX(),(int)ev.getY())){
	...
	/**
	 *  touch listeners have issues when touch events are consumed in the ScrollViews ,e.g  if there is a clickable button in the ScrollView,
	 *  instead call {@link #updatePullState(MotionEvent ev)} instead ...
	 */



	//  when the sv's content is smaller than its height ...   it is not OK ??...
	//   canPullUp:false  sv.getScrollY():0    sv.getChildAt(0).getMeasuredHeight():600      sv.getMeasuredHeight()1118

	//  when the sv's content is larger than its height ...   it is ok...
	//    canPullUp:true   sv.getScrollY():882    sv.getChildAt(0).getMeasuredHeight():2000      sv.getMeasuredHeight()1118
	//    canPullUp:false  sv.getScrollY():601    sv.getChildAt(0).getMeasuredHeight():2000      sv.getMeasuredHeight()1118
	...
	private OnTouchListener topViewTouchListener = new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if ((  svContentSmallerThanSvHeight||svContentLargerThanSvHeight_hit_bottom )&& mCurrentViewIndex == 0)
				...
				canPullUp = false;
			Log.i("ccc","canPullUp:"+canPullUp+"  sv.getScrollY():"+sv.getScrollY()+"    sv.getChildAt(0).getMeasuredHeight():"+sv.getChildAt(0).getMeasuredHeight()+ "      sv.getMeasuredHeight()"+sv.getMeasuredHeight());
			return false;
		}
	};
	...
	private OnTouchListener bottomViewTouchListener = new OnTouchListener() {
		...
		public boolean onTouch(View v, MotionEvent event) {
			...
			if (sv.getScrollY() == 0 && mCurrentViewIndex == 1)
				...
				canPullDown = false;
			Log.i("ccc","canPullDown:"+canPullDown);
			return false;
		}
	};
