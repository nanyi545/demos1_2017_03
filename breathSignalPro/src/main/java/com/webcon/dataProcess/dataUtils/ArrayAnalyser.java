package com.webcon.dataProcess.dataUtils;
...
//        dumpArray(localMin);
    ...
    /**
     * for identifying each breath cycle,findLocalMins() {@link com.webcon.dataProcess.dataUtils.ArrayAnalyser#findLocalMins(int[])}  will fail due to noises.
    ...
    public static int[] findLocalMins_refined(int[] intArray, BreathCycleCondition breathCycleCondition){
        ...
        List<Integer> refinedMinList=new ArrayList<Integer>();  // list to hold refined indexes
        ...
        for (int ii=0;ii<simpleLocalMins_length-1;ii++){
            ...
            if (condA) frontWallCondition=false;
            else {
                ...
                frontWallCondition=(findMax_inIntArray(tempArray)-intArray[simpleLocalMins[ii]])> breathCycleCondition.wallHeight?true:false;
            }
            boolean endWallCondition;
            if (condB) endWallCondition=false;
            else{
                ...
                endWallCondition=(findMax_inIntArray(tempArray2)-intArray[simpleLocalMins[ii]])> breathCycleCondition.wallHeight?true:false;
            }

            // if the breathCycleCondition height is satisfied in both directions
            if (frontWallCondition&&endWallCondition) refinedMinList.add(simpleLocalMins[ii]);
        }

//        -------satisfy the minimum time interval-------
        List<Integer> refinedMinList2=new ArrayList();
        if (refinedMinList.size()>1) {
            refinedMinList2 = refinedWithInterval2(refinedMinList, breathCycleCondition.minimumInterval);
        } else refinedMinList2=refinedMinList;
        ...
        // convert to array
        int[] refinedLocalMins=convert2intArray(refinedMinList2);
        return refinedLocalMins;
    }
...
//            dumpArray(flux_ret);  // print the breath flux values
     ...
     * @return
     */
    public static double findFreqComponent(int[] ints,int startFreqBin,int endFreqBin){
        ...
        double percentage=findSum(tempConversion,startFreqBin,endFreqBin)/findSum(tempConversion,1,128);
        return percentage;
    }


    public static double findFreqComponent(double[] ints,int startFreqBin,int endFreqBin){
        double [] input = ints;
        DftNormalization norm1=DftNormalization.STANDARD;
        FastFourierTransformer transformer=new FastFourierTransformer(norm1);
    ...
    public static double findBreathComponent(int[] ints){
        int[] int256fft=new int[256];
        System.arraycopy(ints,0,int256fft,0,256);
        double percentage=findFreqComponent(int256fft,8,23);
        return percentage;
    }
